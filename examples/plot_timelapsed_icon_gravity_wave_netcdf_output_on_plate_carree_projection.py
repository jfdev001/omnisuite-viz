...
def concatenate_gravity_wave_datasets():
    """Implement the function to concatenate your data!

    you will likely use xarray here...  maybe you can use netCDF4 instead? idk
    """

    # here i am doing a "type annotation" and saying this variable can be
    # EITHER a netcdf4 dataset OR a xarray dataset... if it turns out you
    # just use xarray only, change this type annotation to
    # to something like `my_cool_variable_name: xarr.core.dataset.Dataset`
    # same logic applies if your final data type is netcdf4 Dataset
    concatenated_gravity_wave_dataset: Union[
        nc.Dataset, xarr.core.dataset.Dataset] = None

    return concatenated_gravity_wave_dataset


def main():
    result = concatenate_gravity_wave_datasets()
...
def cli():
    ...
    try:
        ...
        blue_marble_path_required = False
    except KeyError:
        default_blue_marble_path = None
        blue_marble_path_required = True
    ...
    parser.add_argument(
        ...
        required=blue_marble_path_required)
...
class IconDataReader(AbstractReader):
    ...
    def read(self):
        ...
        self._netcdf_response_var_file = nc.Dataset(
        ...
        self._netcdf_height_file = nc.Dataset(self.netcdf_height_file_path)
    ...
    @staticmethod
    def _get_netcdf_long_name_to_netcdf_var_name(dataset: nc.Dataset) -> Dict:
